...
Stepper28BYJ::Stepper28BYJ(uint8_t motor_pin_1, uint8_t motor_pin_2, uint8_t motor_pin_3, uint8_t motor_pin_4)
{
...
void Stepper28BYJ::setSpeed(uint32_t whatSpeed) {
	this->step_delay = whatSpeed;
}

void Stepper28BYJ::stepExactly(uint16_t numberOfSteps) {
	if( this->isEnabled == false ) {
		// return if motor is off
		return;
	}
...
void Stepper28BYJ::stepMotor(uint8_t thisStep) {
	switch (thisStep) {
		case 0:  // 1010
